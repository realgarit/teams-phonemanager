#!/usr/bin/env bash
#
# Generate a self-signed CODE SIGNING identity as a .p12 (same pattern as tertius).
#
# This .p12 is THE signing identity. Reusing the same one for every build keeps
# the codesign designated requirement stable, so macOS treats upgrades as the
# same app (keychain/MSAL token cache and any TCC grants persist across
# `brew upgrade`). Store it as a GitHub Actions secret for CI.
#
# Usage:   P12_PASSWORD=... Scripts/make-signing-cert.sh [output-dir]
# Env:     P12_PASSWORD (required)   CERT_CN (default "Teams Phone Manager Self-Signed")
#
set -euo pipefail

OUTDIR="${1:-./secrets}"
CN="${CERT_CN:-Teams Phone Manager Self-Signed}"
